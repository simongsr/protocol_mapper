...
{{ field|sqlalchemy.association_tablename }} = Table(
    ...
    Column('{{ keyfield.parent|sqlalchemy.modelname }}___{{ keyfield.name }}', {{ keyfield|sqlalchemy.datatype }}, ForeignKey('{{ keyfield.parent|sqlalchemy.modelname }}.{{ keyfield.name }}'), nullable=False),
        {% endfor %}
        {% for keyfield in field.data_type|core.key_fields %}
    Column('{{ field.data_type|sqlalchemy.modelname }}___{{ keyfield.name }}', {{ keyfield|sqlalchemy.datatype }}, ForeignKey('{{ field.data_type|sqlalchemy.modelname }}.{{ keyfield.name }}'), nullable=False),
        {% endfor %}
    {%- set foreign_keys = field|sqlalchemy.association_foreignkeys_dict %}
    ForeignKeyConstraint([{{ foreign_keys.keys()|join(', ') }}], [{{ foreign_keys.values()|join(', ') }}])
)
